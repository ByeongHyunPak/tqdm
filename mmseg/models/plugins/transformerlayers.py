...
# ---------------------------------------------------
# --------- soft attention mask transformer ---------
# ---------------------------------------------------

class SoftAttnMaskAttention(nn.Module):
    ...
    def forward(self, q, k, v, soft_attn_mask):
        B, N, C = q.shape
...
@MODELS.register_module()
class SoftAttnMaskMultiheadAttention(BaseModule):
    ...
    def __init__(self,
        ...
        self.attn = SoftAttnMaskAttention(embed_dims, num_heads, attn_drop,
                                          **kwargs)
...
@MODELS.register_module()
class SoftAttnMaskTransformerLayer(BaseModule):
    def __init__(self,
                 attn_cfgs=None,
                 ffn_cfgs=dict(
                     type='FFN',
    ...
    def forward(self,
                ...
                key_pos=None,
                attn_masks=None,
                soft_attn_masks=soft_attn_masks,
                query_key_padding_mask=None,
                key_padding_mask=None,
                **kwargs):
        norm_index = 0
...
@PLUGIN_LAYERS.register_module()
class SoftAttnMaskTransformerLayerSequence(BaseModule):
    def __init__(self, transformerlayers=None, num_layers=None, init_cfg=None):
        super().__init__(init_cfg)
        if isinstance(transformerlayers, dict):
            transformerlayers = [
