...
@UDA.register_module()
class BASELINE(UDADecorator):
    def __init__(self, **cfg):
        super(BASELINE, self).__init__(**cfg)
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        # src_feat = src_feat[-1] # B, C, H, H

        # with torch.no_grad():
        #     self.get_imnet_model().eval()
        #     feat_imnet = self.get_imnet_model().extract_feat(img)
        #     feat_imnet = [f.detach() for f in feat_imnet]
        #     feat_imnet = feat_imnet[-1]

        # src_feat = nn.MaxPool2d(48)(src_feat)
        # feat_imnet = nn.MaxPool2d(64)(feat_imnet) 

        # src_feat = F.normalize(src_feat, dim=-1)
        # feat_imnet = F.normalize(feat_imnet, dim=-1)

        # sim_mat = torch.matmul(src_feat, feat_imnet.T)
        # mask = torch.eye(src_feat.shape[0], dtype=torch.bool).to(src_feat.device)
        
        # positives = sim_mat[mask.bool()].view(src_feat.shape[0])
        # negatives = sim_mat[~mask.bool()].view(src_feat.shape[0])

        # logits = torch.cat([positives, negatives], dim=1)

        # cnt_loss = nn.CrossEntropyLoss().to(src_feat.device)(logits, mask)
