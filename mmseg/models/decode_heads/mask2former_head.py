...
class Mask2FormerHead(BaseDecodeHead):
    ...
    def forward(self, feats, img_metas, return_mask_features=False, get_similarity=False):
        """Forward function.
    ...
    def forward_train(self, x, img_metas, gt_semantic_seg, train_cfg,
                      gt_labels, gt_masks, ):
    ...
    def forward_test(self, inputs, img_metas, test_cfg):
        ...
        # cls_score, mask_pred = all_cls_scores[self.query_index], all_mask_preds[self.query_index]
    ...
    def forward_inference(self, inputs, img_metas, test_cfg):
        all_cls_scores, all_mask_preds, mask_features =\
            self(inputs, img_metas, return_mask_features=True)
        return all_mask_preds, mask_features

    def forward_mask_features(self, inputs, img_metas, test_cfg):
        # pre_mask_features = self.pixel_decoder.input_convs[2](inputs[1])
        # pre_mask_features = inputs[1]
        mask_features, multi_scale_memorys, save_features = self.pixel_decoder.forward_feats(inputs, 0)
        pre_mask_features = save_features[-1]
        post_mask_features = multi_scale_memorys[-1] # mask_features

        return pre_mask_features, post_mask_features
    
    def get_similarity(self, inputs, img_metas, test_cfg):
        all_cls_scores, all_mask_preds = self(inputs, img_metas, get_similarity = True)
